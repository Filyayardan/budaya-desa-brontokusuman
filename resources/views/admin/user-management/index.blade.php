...
@section('title', 'Kelola Subadmin')
@section('header', 'Kelola Subadmin')

@section('content')
<div class="flex items-center justify-between mb-6">
    <p class="text-gray-500 text-sm">Total: {{ $subAdmin->total() }} Sub-admin</p>
    <a href="{{ route('admin.userManagement.create') }}" class="px-4 py-2 rounded-lg text-white text-sm font-medium" style="background: linear-gradient(135deg, #d4a017, #b8860b);">
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse($subAdmin as $i => $p)
            <tr class="hover:bg-gray-50">
                <td class="px-5 py-4 text-gray-500">{{ $subAdmin->firstItem() + $i }}</td>
                {{-- <td class="px-5 py-4">
                    @if($p->foto)
                ...
                <td class="px-5 py-4 font-medium text-gray-900">{{ $p->username }}</td>
                <td class="px-5 py-4 text-gray-600">{{ $p->password }}</td>

                <td class="px-5 py-4">
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('admin.userManagement.edit', $p) }}" class="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-xs font-medium hover:bg-blue-100"><i class="fas fa-edit mr-1"></i>Edit</a>
                        <form action="{{ route('admin.userManagement.destroy', $p) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">
                            @csrf @method('DELETE')
