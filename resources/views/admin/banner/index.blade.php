...
@section('content')
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.banner.create') }}" class="px-4 py-2.5 rounded-lg text-white text-sm font-medium"
                ...
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div class="relative h-48 bg-gradient-to-br from-gold-600/20 to-dark-700">
                        ...
                        <div class="absolute top-3 right-3">
                            @if ($b->aktif)
                                <span
                                    class="px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700">Aktif</span>
                            @else
                                <span
                                    class="px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-500">Nonaktif</span>
                            @endif
                        </div>
                    </div>
                    <div class="p-5">
                        @if ($b->badge)
                            <span
                                class="inline-block px-2.5 py-1 rounded-full text-xs font-medium bg-gold-50 text-gold-700 mb-2">{{ $b->badge }}</span>
                        @endif
                        <h3 class="font-semibold text-gray-900 text-lg mb-1">{{ $b->judul_atas ?? '' }}
                            {{ $b->judul_tengah ?? '' }} {{ $b->judul_bawah ?? '' }}</h3>
                        @if ($b->deskripsi)
                            <p class="text-gray-500 text-sm line-clamp-2 mb-3">{!! nl2br(e($b->deskripsi)) !!}</p>
                        @endif
    ...
    @else
        <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <a href="{{ route('admin.banner.create') }}" class="px-4 py-2 rounded-lg text-white text-sm font-medium"
                style="background: linear-gradient(135deg, #d4a017, #b8860b);">
