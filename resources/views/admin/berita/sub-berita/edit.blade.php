...
@section('title', 'Edit Sub Berita')
@section('header', 'Edit Sub Berita')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form action="{{ route('admin.berita.sub-berita.update', [$berita, $subBerita]) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Judul Sub Berita <span class="text-red-500">*</span></label>
                <input type="text" name="judul_sub" value="{{ old('judul_sub', $subBerita->judul_sub) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-gold-500 outline-none">
            </div>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Urutan</label>
                    <input type="number" name="urutan" value="{{ old('urutan', $subBerita->urutan) }}" min="0" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-gold-500 outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Gambar Utama</label>
                    @if($subBerita->gambar)
                    <div class="relative group w-fit mb-2">
                        <img src="{{ asset('storage/' . $subBerita->gambar) }}" class="w-20 h-20 rounded-lg object-cover border border-gray-200">
            ...
            <div class="mb-4">
                ...
                @if(!empty($subBerita->galeri))
                <div class="galeri-existing flex flex-wrap gap-2 mb-2">
                    @foreach($subBerita->galeri as $idx => $path)
                ...
                <p class="mt-1 text-xs text-gray-400">Tulis <b>[foto1]</b>, <b>[foto2]</b>, dst. di dalam isi sub berita untuk menampilkan foto di posisi yang diinginkan. Centang silang untuk menghapus foto.</p>
            ...
            <div class="mb-6">
                ...
                <textarea name="isi_sub" rows="10" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-gold-500 outline-none">{{ old('isi_sub', $subBerita->isi_sub) }}</textarea>
