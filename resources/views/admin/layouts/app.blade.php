    ...
    <div class="flex min-h-screen">
        <aside id="sidebar" class="w-64 bg-dark-900 text-white flex flex-col fixed h-full z-40 transition-transform -translate-x-full lg:translate-x-0">
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium {{ request()->routeIs('admin.dashboard') ? 'bg-gold-500/10 text-gold-300' : 'text-gray-400 hover:bg-dark-800 hover:text-white' }}">
                    <i class="fas fa-tachometer-alt w-5"></i><span>Dashboard</span>
                </a>
                <div class="pt-3 pb-1 px-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Konten</div>
                @php
                    $menu = [
                ...
                <a href="{{ route($item['route']) }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium {{ request()->routeIs($pattern) ? 'bg-gold-500/10 text-gold-300' : 'text-gray-400 hover:bg-dark-800 hover:text-white' }}">
            ...
            <div class="p-4 border-t border-gold-500/10">
                <a href="{{ route('home') }}" target="_blank" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-400 hover:bg-dark-800 hover:text-white">
                    <i class="fas fa-external-link-alt w-5"></i><span>Lihat Website</span>
                </a>
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-400 hover:bg-red-500/10 hover:text-red-400">
        ...
        <div class="flex-1 lg:ml-64">
            ...
            <main class="p-6">
                @if(session('success'))
                <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm flex items-center">
                ...
                <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
                    <div class="flex items-center mb-2"><i class="fas fa-exclamation-circle mr-2"></i><strong>Terjadi kesalahan:</strong></div>
