...
<section class="py-16 bg-pattern">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-wrap gap-3 justify-center mb-12">
            <a href="{{ route('budaya') }}" class="px-5 py-2.5 rounded-full text-sm font-medium {{ !isset($kategoriAktif) ? 'gradient-gold text-dark-950' : 'bg-dark-800 text-gray-300 hover:bg-gold-500/10 hover:text-gold-300 border border-gold-500/10' }} transition-all">
                Semua
            </a>
            @foreach($kategori as $k)
            <a href="{{ route('budaya.kategori', $k->id) }}" class="px-5 py-2.5 rounded-full text-sm font-medium {{ (isset($kategoriAktif) && $kategoriAktif->id == $k->id) ? 'gradient-gold text-dark-950' : 'bg-dark-800 text-gray-300 hover:bg-gold-500/10 hover:text-gold-300 border border-gold-500/10' }} transition-all">
                {{ $k->nama_kategori }}
            </a>
            @endforeach
        </div>
                ...
                <div class="bg-white backdrop-blur rounded-2xl overflow-hidden border border-gold-500/10 hover:border-gold-500/30 h-full">
                    <div class="relative aspect-[3/2]  bg-gradient-to-br from-gold-600/20 to-dark-700 overflow-hidden">
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="gradient-gold text-dark-950 text-xs font-bold px-3 py-1 rounded-full">{{ $b->kategori->nama_kategori ?? '-' }}</span>
                        </div>
                        @if($b->unggulan)
                        <div class="absolute top-4 right-4">
                            <span class="bg-dark-900/80 text-gold-300 text-xs font-bold px-3 py-1 rounded-full border border-gold-500/20"><i class="fas fa-star mr-1"></i>Unggulan</span>
                        </div>
                        @endif
                    </div>
                    <div class="p-6">
