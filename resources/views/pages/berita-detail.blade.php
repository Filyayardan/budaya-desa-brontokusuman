    ...
    <section class="header-section relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav class="flex items-center text-sm text-gray-500 mb-8">
                ...
                <i class="fas fa-chevron-right text-xs mx-3 text-main_txt-500/40"></i>
                <span class="text-main_txt-300">{{ Str::limit($berita->judul, 40) }}</span>
            </nav>
            <h1 class="font-display text-4xl sm:text-5xl font-bold text-main_txt mb-4">{{ $berita->judul }}</h1>
    ...
    <section class="py-16 bg-pattern">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-second_bg backdrop-blur rounded-2xl border border-gold-500/10 p-8 sm:p-10">
                <div
                    class="prose prose-invert max-w-none text-tertiary leading-relaxed prose-headings:text-white prose-a:text-gold-400 prose-strong:text-white">
                    {!! nl2br(e($berita->isi)) !!}
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        @foreach ($beritaLain as $bl)
                            <a href="{{ route('berita.detail', $bl->id) }}" class="card-hover group">
                                <div
                                    class="bg-white backdrop-blur rounded-xl overflow-hidden border border-gold-500/10 hover:border-gold-500/30">
                                    <div
                                        class="relative h-40 bg-gradient-to-br from-gold-600/20 to-dark-700 overflow-hidden">
                                        @if ($bl->gambar)
                                            <img src="{{ asset('storage/' . $bl->gambar) }}"
                                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                        @else
                                            <div class="absolute inset-0 flex items-center justify-center"><i
                                                    class="fas fa-newspaper text-4xl text-gold-500/20"></i></div>
                                        @endif
                                    </div>
                                    <div class="p-5">
                                        <h3
                                            class="font-display text-lg font-bold text-main_txt group-hover:text-gold-300 transition-colors mb-2">
                                            {{ $bl->judul }}</h3>
                                        <span class="text-gray-500 text-xs"><i
                                                class="far fa-clock mr-1"></i>{{ $bl->created_at->translatedFormat('d M Y') }}</span>
                                    </div>
                                </div>
                            </a>
                        @endforeach
